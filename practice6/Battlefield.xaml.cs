...
namespace practice6
{
    ...
    public sealed partial class Battlefield : Page
    {
        ...
        readonly BitmapImage _longShip1 = new BitmapImage(new Uri("ms-appx:///Assets/Textures/longship1.png")),
                ...
                _wave = new BitmapImage(new Uri("ms-appx:///Assets/Textures/wave.png"));
        Random _rnd;
        ...
        void InitializeFields()
        {
            ...
            SolidColorBrush color = new SolidColorBrush(Windows.UI.Colors.DodgerBlue);
            ...
            for (int i = 0; i < SeaField._xDim; i++)
            {
                ...
                {
                    EnemyField.Children.Add(CreateButton(i, j, color, ref sb, margin2, buttonHeight, buttonWidth, true));
                    PlayerField.Children.Add(CreateButton(i, j, color, ref sb, margin2, buttonHeight, buttonWidth, false));
                }
            }
        }

        Button CreateButton(int row, int col, Brush background,
        ...
        void InitializeElements()
        {
            double windowHeight = Window.Current.Bounds.Height,
                ...
                marginH = windowWidth / 100, marginV = windowHeight / 100;
            ...
            for (int i = 0; i < EnemyShipsGrid.Children.Count; i++)
            {
                ...
                (ShipCloneCanvas.Children[i] as Image).Height = relPanel.Height;
                (ShipCloneCanvas.Children[i] as Image).Width = relPanel.Height * (i + 1);
            }

            EnemyRelPanel.Width = windowWidth * 0.75;
            EnemyRelPanel.Height = (windowHeight - (EnemyShips.Height * 2) - marginV * 4) / 2;
            Canvas.SetTop(EnemyRelPanel, EnemyShips.Height + marginV);

            PlayerRelPanel.Width = windowWidth * 0.75;
            PlayerRelPanel.Height = (windowHeight - (EnemyShips.Height * 2) - marginV * 4) / 2;
            Canvas.SetTop(PlayerRelPanel, EnemyShips.Height + marginV + EnemyRelPanel.Height + marginV * 2);

            BackToMenuButton.Width = WinLoseBanner.Width / 2;
            BackToMenuButton.Height = WinLoseBanner.Height / 5;

            LeftAnnBar.Height = windowHeight - EnemyShips.Height * 2;
            LeftAnnBar.Width = (windowWidth - EnemyRelPanel.Width - marginH * 2) / 2;

            RightAnnBar.Height = windowHeight - EnemyShips.Height * 2;
            RightAnnBar.Width = (windowWidth - PlayerRelPanel.Width - marginH * 2) / 2;
            Canvas.SetLeft(RightAnnBar, LeftAnnBar.Width + marginH + EnemyRelPanel.Width + marginH);

            Canvas.SetLeft(PlayerRelPanel, LeftAnnBar.Width + marginH);
            Canvas.SetLeft(EnemyRelPanel, LeftAnnBar.Width + marginH);
        ...
        private void CheckWinLoseCondition(IEnumerable<Point> collection) // todo: refactor to gameinfo class
        {
            ...
            if (list.Count == 0)
            {
                ...
                {
                    WinLoseBackground.ImageSource = _lose;
                    LoseSound.Play();
                    WinLoseText.Text = "You lose!";
                }
                else if (list == _enemy.ShipPoints)
                {
                    WinLoseBackground.ImageSource = _win;
                    WinSound.Play();
        ...
        void DrawHit(Point where, bool isEnemyField, bool isSucessfull)
        {
            ...
            Image image = new Image
            {
                Width = measurement,
                Height = measurement,
                Source = isSucessfull ? _explosion : _wave,
            };
        ...
        private void GridKeyDown(object sender, KeyRoutedEventArgs e)
        {
            ...
            {
                WinSound.Source = new Uri("ms-appx:///Assets/Sounds/win_sound_NSFW.wav");
            }
        }
    }
}
